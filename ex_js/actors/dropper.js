...
DropperActor.prototype.update = function() {
	...
	if(!this.started) {
		...
		if(this.lastY > this.absPosition.y)		this.progress += (this.lastY-this.absPosition.y);
		this.lastY = this.absPosition.y;
	}
...
DropperActor.prototype.tryDrop = function() {
	for(var i in this.loadedItems) {
		...
		if(this.loadedItems[i].stage == this.stage) {
			...
			else if(this.loadedItems[i].subStep > this.subStep)		return;
		}
		else if(this.loadedItems[i].stage > this.stage)		return;
	}
};
DropperActor.prototype.dropLoaded = function(item) {
	...
	if(actor != null) {
		...
		if(typeof actor.loadingData === "function")		actor.loadingData(payload.data);
